...
class Barbs:
	...
	def checkTurn(self, iGameTurn):
		...
		if utils.isYearIn(-210, 300):
			self.checkSpawn(iBarbarian, iHorseman, 3 + iHandicap, (110, 58), (129, 65), self.spawnNomads, iGameTurn, 8-iHandicap, 0, ["TXT_KEY_ADJECTIVE_XIONGNU"])
		...
		if utils.isYearIn(-1600, -850):
			...
			self.checkSpawn(iBarbarian, iHorseArcher, 2 + iHandicap, (91, 50), (103, 58), self.spawnNomads, iGameTurn, 8-iHandicap, 2, ["TXT_KEY_ADJECTIVE_TURKIC"])
		elif utils.isYearIn(700, 1040):
			#if utils.getScenario() == i3000BC:  #late start condition
			self.checkSpawn(iBarbarian, iHorseArcher, 2 + iHandicap, (91, 50), (106, 62), self.spawnNomads, iGameTurn, 6-iHandicap, 2, ["TXT_KEY_ADJECTIVE_TURKIC"])
		...
		if utils.isYearIn(1200, 1650):
			if not utils.getAreaCitiesCiv(iMongolia, utils.getPlotList((82, 58), (95, 70))):
				self.checkSpawn(iBarbarian, iSteppeUnit, 2 + iExtra, (88, 57), (94, 61), self.spawnNomads, iGameTurn, 10-iHandicap, 5, ["TXT_KEY_ADJECTIVE_TATAR", "TXT_KEY_ADJECTIVE_NOGAI"])
		if utils.isYearIn(1400, 1700):
			if utils.getAreaCities(utils.getPlotList((91, 58), (106, 62))):
				self.checkSpawn(iBarbarian, iSteppeUnit, 1 + iExtra, (91, 58), (106, 62), self.spawnNomads, iGameTurn, 10-2*iHandicap, 2, ["TXT_KEY_ADJECTIVE_UZBEK", "TXT_KEY_ADJECTIVE_KAZAKH"])
		if utils.isYearIn(1500, 1800):
			if utils.getAreaCities(utils.getPlotList((96, 62), (110, 70))):
				self.checkSpawn(iBarbarian, iSteppeUnit, 1 + iExtra, (96, 62), (110, 70), self.spawnNomads, iGameTurn, 8-iHandicap, 2, ["TXT_KEY_ADJECTIVE_SIBIR"])
		...
		if utils.isYearIn(-210, 50):
			...
			self.checkSpawn(iBarbarian, iCamelArcher, 1, (60, 33), (78, 42), self.spawnNomads, iGameTurn, 8-iHandicap, 4, ["TXT_KEY_ADJECTIVE_BERBER"])
			
		#camels in arabia
		if utils.isYearIn(190, 550):
			self.checkSpawn(iBarbarian, iCamelArcher, 2, (85, 36), (96, 42), self.spawnNomads, iGameTurn, 9-iHandicap, 7, ["TXT_KEY_ADJECTIVE_BEDOUIN"])
		...
		if utils.isYearIn(1300, 1600):
			if iGameTurn % 18 == 0:
				if not gc.getMap().plot(29, 34).isUnit():
					utils.makeUnitAI(iDogSoldier, iNative, (29, 34), UnitAITypes.UNITAI_ATTACK, 3 + iHandicap)
			elif iGameTurn % 18 == 9:
				if not gc.getMap().plot(30, 13).isUnit():
					utils.makeUnitAI(iDogSoldier, iNative, (33, 12), UnitAITypes.UNITAI_ATTACK, 3 + iHandicap)
		
		if utils.isYearIn(1700, 1900):
			self.checkSpawn(iNative, iMountedBrave, 1 + iHandicap, (14, 52), (23, 62), self.spawnUprising, iGameTurn, 12 - iHandicap, 2)
		...
		if utils.isYearIn(1500, 1650):
			...
			self.checkSpawn(iNative, iMohawk, 1 + iHandicap, (24, 54), (31, 61), self.spawnUprising, iGameTurn, 8, 4)
